...
const config = {
    A187: 'HeheBot',
    instagram: 'fellipe_edit_world',
    nomer: 'wa.me/994405327962',
    aktif: '24 JAM',
    youtube: 'https:OFF',
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = `acha que eu tenho bola de cristal pra saber a resolução? \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `ta ai meu mano *${value}* querida ♥️\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
